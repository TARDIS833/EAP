# -*- coding: utf-8 -*-
"""엑셀 자동 변환 (자사몰/스마트스토어) - excel_auto_ver8_1."""
...
def download_bytes(url: str, timeout_sec: int = 15) -> bytes:
    req = Request(url, headers={"User-Agent": f"excel-auto-updater/{VERSION.replace('_', '.')}"} )
...
def list_backup_config_snapshots() -> List[Path]:
    ...
    for entry in backup_root.iterdir():
        if not entry.is_dir():
            continue
        if (entry / "configs").is_dir():
            snapshots.append(entry)
    snapshots.sort(key=lambda p: p.name, reverse=True)
    return snapshots


def restore_configs_from_backup(snapshot_dir: Path) -> Tuple[bool, str]:
    source_configs = snapshot_dir / "configs"
    if not source_configs.is_dir():
        return False, f"복원 실패: 백업 경로에 configs가 없습니다.\n- 경로: {snapshot_dir}"

    config_dir = get_app_dir() / "configs"
    config_dir.mkdir(parents=True, exist_ok=True)
    current_backup = backup_configs_dir(config_dir)
    ...
    try:
        ...
        return True, (
            f"백업 복원 완료\n"
            f"- 복원본: {snapshot_dir}\n"
            f"- 현재본 백업: {current_backup}"
        )
    except Exception as e:
        try:
            rollback_source = current_backup / "configs"
            if rollback_source.is_dir():
                if config_dir.exists():
                    shutil.rmtree(config_dir)
                shutil.copytree(rollback_source, config_dir)
                load_mappings()
        except Exception as rollback_e:
            logger.exception("복원 실패 후 롤백도 실패: %s", rollback_e)
...
def apply_online_patch_update() -> Tuple[bool, str]:
    ...
    min_app_version = normalize_text(manifest.get("min_app_version"))
    if min_app_version and parse_version_text(VERSION) < parse_version_text(min_app_version):
        return False, (
            "패치 적용 불가: 현재 앱 버전이 최소 요구 버전보다 낮습니다.\n"
            f"- 현재: {VERSION}\n"
            f"- 최소: {min_app_version}"
        )
...
def split_color_size(value: str) -> Tuple[str, str]:
    ...
    # 마쉬멜로우핑크_M 형태(언더스코어 구분)를 우선 처리한다.
    m = re.match(r"^(.+?)_(XXL|XL|L|M|S|XS|FREE)$", value, flags=re.IGNORECASE)
    if m:
        return normalize_text(m.group(1)), normalize_text(m.group(2)).upper()
    if "사이즈=" in value:
...
CAMI_B_PREFIX_ALLOW_COLORS = {"블랙", "스킨"}


def normalize_output_name(output_name: str) -> str:
    text = normalize_text(output_name)
    m = re.match(r"^(\[B\])?캐미젤리B_([^_]+)_(.+)$", text)
    if not m:
        return text

    b_prefix = m.group(1) or ""
    color = normalize_text(m.group(2))
    size = normalize_text(m.group(3))

    # 잘못 생성된 M_FREE/L_FREE/S_FREE 꼬리 제거
    if "_" in size and size.upper().endswith("_FREE"):
        size = size[:-5]
    ...
    # [B]는 블랙/스킨만 허용
    if color not in CAMI_B_PREFIX_ALLOW_COLORS:
        b_prefix = ""

    return f"{b_prefix}캐미젤리B_{color}_{size}"
...
def extract_gift_events(
    ...
    for rule in rules:
        ...
        if needle_cmp and needle_cmp in product_cmp:
            events.append({"rule": rule, "source": product_norm, "source_type": "product"})
    ...
    for tok in tokens:
        ...
        for rule in rules:
            ...
            if needle_cmp and needle_cmp in tok_cmp:
                events.append({"rule": rule, "source": tok_display, "source_type": "option"})
                matched = True
                break
        if matched:
            continue
    ...
    # option 기반 증정이 존재하면 동일 output.product의 product 기반 증정은 제거한다.
    option_products = set()
    for ev in events:
        if ev.get("source_type") != "option":
            continue
        out = ev.get("rule", {}).get("output", {})
        if isinstance(out, dict):
            p = normalize_text(out.get("product"))
            if p:
                option_products.add(p)
    if option_products:
        filtered: List[Dict[str, Any]] = []
        for ev in events:
            out = ev.get("rule", {}).get("output", {})
            p = normalize_text(out.get("product")) if isinstance(out, dict) else ""
            if ev.get("source_type") == "product" and p in option_products:
                continue
            filtered.append(ev)
        events = filtered
...
def build_gift_rows_from_events(
    ...
    for ev in events:
        ...
        seen.add(key)
        row = dict(base_row)
        row["출고명"] = normalize_output_name(f"{p}_{c}_{s}".strip("_"))
...
def build_self_rows(
    ...
    for _, r in df.iterrows():
        ...
        def append_main_row(prefix: str, color: str, size: str, row_qty: str) -> None:
            out = dict(base_row)
            out["출고명"] = normalize_output_name(f"{prefix}_{color}_{size}".strip("_"))
    ...
    for _, r in df.iterrows():
        ...
        if custom_rule:
            ...
            for item in custom_rule:
                ...
                if size and size.upper() != "FREE" and "(증정)" not in size:
                    selected_size = size
                output_name = normalize_output_name(f"{prefix}_{color}_{size}".strip("_"))
        ...
        if not parsed:
            ...
            if manual:
                ...
                for m in manual:
                    ...
                    if m.size and m.size.upper() != "FREE" and "(증정)" not in m.size:
                        selected_size = m.size
                    output_name = normalize_output_name(f"{m.product}_{m.color}_{m.size}".strip("_"))
...
def main():
    ...
    if tk is None or (sys.platform not in ("win32", "darwin") and not os.environ.get("DISPLAY")):
        ...
        return
    # 시작 시 최신 configs 패치를 자동 반영한다. 실패해도 앱은 계속 실행한다.
    try:
        ok, msg = apply_online_patch_update()
        if ok:
            logger.info("[자동패치] %s", msg.replace("\n", " | "))
        else:
            logger.warning("[자동패치] 미적용: %s", msg.replace("\n", " | "))
    except Exception as e:
        logger.warning("[자동패치] 예외 발생: %s", e)
    run_gui()
...
def run_gui():
    ...
    def ask_backup_snapshot() -> Optional[Path]:
        snapshots = list_backup_config_snapshots()
        if not snapshots:
            messagebox.showinfo("백업 복원", "복원 가능한 백업이 없습니다.", parent=root)
            return None
        ...
        btn = tk.Frame(win)
        btn.pack(fill="x", padx=12, pady=(6, 12))

        def on_confirm():
            idx = listbox.curselection()
            if not idx:
                messagebox.showwarning("백업 복원", "백업 시점을 선택해주세요.", parent=win)
                return
            selected["path"] = snapshots[idx[0]]
            win.destroy()

        def on_cancel():
            win.destroy()

        tk.Button(btn, text="취소", command=on_cancel, width=10).pack(side="right")
        tk.Button(btn, text="복원", command=on_confirm, width=10).pack(side="right", padx=6)

        win.wait_window()
        return selected["path"]

    def run_restore_from_backup():
        snapshot = ask_backup_snapshot()
        if snapshot is None:
            return
        if not messagebox.askyesno(
            "백업 복원 확인",
            "현재 configs가 선택한 백업으로 교체됩니다.\n계속 진행할까요?",
            parent=root,
        ):
            return
        ok, msg = restore_configs_from_backup(snapshot)
        if ok:
            messagebox.showinfo("백업 복원", msg, parent=root)
            result_status.set("백업 복원 완료")
            result_detail.set("선택한 시점의 규칙으로 복원되었습니다. 변환을 다시 실행해주세요.")
        else:
            messagebox.showerror("백업 복원 실패", msg, parent=root)
    ...
    tk.Button(
        patch_row,
        text="백업에서 복원",
        command=run_restore_from_backup,
        padx=12,
        bg="#eef1f5",
        fg="#2e3b4e",
        relief="solid",
